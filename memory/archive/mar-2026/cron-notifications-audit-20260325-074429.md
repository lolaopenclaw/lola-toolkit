...
- 📤 **Telegram message calls:**
```bash
187:    /home/mleon/.npm-global/bin/openclaw message send \
```
- ⚠️ No quiet hours check (sends messages without time check)
...
- 📤 **Telegram message calls:**
```bash
74:    # Replace openclaw message send patterns
76:        -e "s|openclaw message send --channel telegram --target \"\$ALERT_CHANNEL\"|openclaw message send --channel telegram --target \"-1003768820594\" --topic $topic|g" \
77:        -e "s|openclaw message send --channel telegram --target .\$.*|openclaw message send --channel telegram --target \"-1003768820594\" --topic $topic \\\\|g" \
78:        -e "s|openclaw message send \"\$MESSAGE\"|openclaw message send --channel telegram --target \"-1003768820594\" --topic $topic --message \"\$MESSAGE\"|g" \
104:    sed -i 's|openclaw message send --channel telegram --target "$ALERT_CHANNEL"|openclaw message send --channel telegram --target "-1003768820594" --topic 29|g' "$SCRIPT"
```
- ✅ Has quiet hours awareness
...
- ❌ **Hardcoded personal chat target (`6884477`)**
```
4:# - Replace "to": "6884477" with topic routing
75:  if (.delivery.to == "6884477" or .delivery.channel == "last") then
81:        if .to == "6884477" then
```
- ✅ Silent script (no notifications)
...
- 📤 **Telegram message calls:**
```bash
206:    /home/mleon/.npm-global/bin/openclaw message send \
```
- ✅ Has quiet hours awareness
...
- 📤 **Telegram message calls:**
```bash
355:        openclaw message send --channel telegram --target "-1003768820594" --topic 29 \
```
- ✅ Has quiet hours awareness
- 🌙 **Likely night script** (by name pattern)
...
- 📤 **Telegram message calls:**
```bash
53:openclaw message send --channel telegram --target "-1003768820594" --topic 25 --message "$MESSAGE"
```
- ✅ Has quiet hours awareness
...
- ✅ Has quiet hours logic:
```
341:# Check quiet hours (00:00-07:00 Madrid)
346:    # During quiet hours: only alert if CRITICAL findings
```
- ✅ Uses topic routing
